...
@RestController(value = "BMS_statement_borrowingController")
@RequestMapping(value = "/actions/bms_statement_borrowing")
public class BMS_statement_borrowingControllerImpl implements BMS_statement_borrowingController {
    protected Logger
    ...
    // 本地控制实现
    @Autowired
    private BMS_statement_borrowingController actionComponent;
    ...
    public CIPResponseMsg repayData(HttpServletRequest request) {
        ...
        CIPResponseMsg msg = null;
        try {
            msg = actionComponent.repayData(request);
        } catch (CIPServiceException e) {
            log.error(e.getMessage(), e);
            CIPErrorCode error = e.getErrorCode();
            msg = new CIPResponseMsg();
