...
public abstract class BMS_statement_repayment_logDaoImpl implements BMS_statement_repayment_logDao {

    // dataMapper
    protected BeanPropertyRowMapper<BMS_statement_repayment_logData> dataRM;
    // poMapper
    protected BeanPropertyRowMapper<BMS_statement_repayment_logPO> poRM;
    ...
    public BMS_statement_repayment_logDaoImpl() {
        this.tableName = "bms_statement_repayment_log";
        this.tableKeys = new String[]{
                "serial_no"
        };
        this.dataRM = BeanPropertyRowMapper.newInstance(BMS_statement_repayment_logData.class);
        this.poRM = BeanPropertyRowMapper.newInstance(BMS_statement_repayment_logPO.class);
    ...
    public BMS_statement_repayment_logPO getSingle(Object... keys) {
        ...
        try {
            ...
            sql.delete(sql.length() - 5, sql.length());
            List<BMS_statement_repayment_logPO> pos = jdbcTemplate.query(sql.toString(), keys, poRM);
            if (pos != null && pos.size() > 0)
                return pos.get(0);
