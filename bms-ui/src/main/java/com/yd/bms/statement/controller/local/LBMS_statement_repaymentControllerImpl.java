...
import com.yd.bms.statement.controller.BMS_statement_repaymentController;
...
import com.yd.common.data.CIPUpdateParameter;
...
@Component
public class LBMS_statement_repaymentControllerImpl  implements BMS_statement_repaymentController{

    // Service
    @Autowired
    protected BMS_statement_repaymentService dataService;
    ...
    ******************************************************************/
    public CIPResponseMsg addData( BMS_statement_repayment_addVO vo){
    ...
    ******************************************************************/
    public CIPResponseMsg deleteData( BMS_statement_repaymentKey key){
    ...
    ******************************************************************/
    public CIPResponseMsg getData( BMS_statement_repaymentKey key){
		...
		if(!StringUtils.isEmpty(templateName)){
			try {
				...
				if(templateFile.exists()){
					...
					SXSSFWorkbook wb = ExcelSheetParser.createWorkBook(templateName, BMS_statement_repaymentConstant.getExcelTitle(), null);
					wb.write(out);
				}
			} catch (Exception e) {
    ...
    public CIPResponseMsg updateData( HttpServletRequest request){
       ...
       CIPResponseMsg msg = new CIPResponseMsg();
        ...
        if(xAuth){
        	...
        	BMS_statement_repaymentKey keys = JSONUtils.convertDataJson2Object(parameter.keys, BMS_statement_repaymentKey.class);
