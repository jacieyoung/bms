...
import com.yd.bms.org.service.impl.BMS_org_profileServiceImpl;
...
@Service(value = "BMS_org_profileServiceImpl")
public class LBMS_org_profileServiceImpl extends BMS_org_profileServiceImpl {
    ...
    public String exportData(String exportPath, CIPReqCondition[] conditions, CIPRuntimeOperator user) {
        ...
        if (os.startsWith("win") || os.startsWith("Win")) {
            ...
            // 2. 生成文件名
            fileName = exportPath + File.separator + "BMS_org_profile"
            ...
            try {
                ...
                fileName = "BMS_org_profile" + DateUtils.getDate(new Date());
                String cmd = null;
    ...
    public void importData(File file, CIPRuntimeOperator user) {
        ...
        List<BMS_org_profilePO> pos = new ArrayList<BMS_org_profilePO>();

        ExcelReader excelReader = new ExcelReader(file);
        try {
            excelReader.openFile();//打开文件
            BMS_org_profilePO po;
            List<Object[]> listResult = excelReader.getAllRow();//读取Excel的数据
            Object[] array;
            int size = listResult.size();
            Field field;
            Map<String, String> titles = BMS_org_profileConstant.getExcelTitle();
            int j = 0;
            Set<String> columns;
            String typeName;
    ...
    public List<CIPDomainValueData> getDomain(String tableId, String domainId) {
        return dataDao.getDomainValues(tableId,domainId);
    }

    @Override
    public List<CIPDomainValueData> loadAllOrg() {
        return dataDao.loadAllOrg();
    }
}
