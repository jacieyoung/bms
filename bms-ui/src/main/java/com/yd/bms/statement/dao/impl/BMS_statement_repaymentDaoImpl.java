...
public abstract class BMS_statement_repaymentDaoImpl implements BMS_statement_repaymentDao {

    // dataMapper
    protected BeanPropertyRowMapper<BMS_statement_repaymentData> dataRM;
    // poMapper
    protected BeanPropertyRowMapper<BMS_statement_repaymentPO> poRM;
    // tableName
    ...
    public BMS_statement_repaymentDaoImpl() {
        this.tableName = "bms_statement_repayment";
        this.tableKeys = new String[]{
                "serial_no"
        };
        this.dataRM = BeanPropertyRowMapper.newInstance(BMS_statement_repaymentData.class);
        this.poRM = BeanPropertyRowMapper.newInstance(BMS_statement_repaymentPO.class);
    ...
    public BMS_statement_repaymentPO getSingle(Object... keys) {
        ...
        try {
            ...
            sql.delete(sql.length() - 5, sql.length());
            List<BMS_statement_repaymentPO> pos = jdbcTemplate.query(sql.toString(), keys, poRM);
            if (pos != null && pos.size() > 0)
                return pos.get(0);
