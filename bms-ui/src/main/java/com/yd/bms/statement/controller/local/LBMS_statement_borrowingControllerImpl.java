...
import com.yd.bms.statement.controller.BMS_statement_borrowingController;
...
@Component
public class LBMS_statement_borrowingControllerImpl implements BMS_statement_borrowingController {

    // Service
    @Autowired
    protected BMS_statement_borrowingService dataService;
     ...
     ******************************************************************/
    public CIPResponseMsg addData(BMS_statement_borrowing_addVO vo) {
     ...
     ******************************************************************/
    public CIPResponseMsg deleteData(BMS_statement_borrowingKey key) {
     ...
     ******************************************************************/
    public CIPResponseMsg getData(BMS_statement_borrowingKey key) {
        ...
        if (!StringUtils.isEmpty(templateName)) {
            try {
                ...
                if (templateFile.exists()) {
                    ...
                    SXSSFWorkbook wb = ExcelSheetParser.createWorkBook(templateName, BMS_statement_borrowingConstant.getExcelTitle(), null);
                    wb.write(out);
                }
            } catch (Exception e) {
    ...
    public CIPResponseQueryMsg searchData(CIPReqParameter parameter) {
        ...
        if (conditions != null && conditions.length > 0) {
            for (CIPReqCondition condition : conditions) {
                if(BMS_statement_borrowingConstant.C_FIELD_SQR_NAME.equals(condition.getFieldName())){
    ...
    public CIPResponseMsg updateData(HttpServletRequest request) {
        ...
        if (xAuth) {
            ...
            BMS_statement_borrowingKey keys = JSONUtils.convertDataJson2Object(parameter.keys, BMS_statement_borrowingKey.class);
    ...
    public CIPResponseMsg repayData(HttpServletRequest request) {
        ...
        if (xAuth) {
            ...
            BMS_statement_borrowingKey keys = JSONUtils.convertDataJson2Object(parameter.keys, BMS_statement_borrowingKey.class);
            Map<String, Object> fields = JSONUtils.convertJson2Map(parameter.fields);
            dataService.repayData(keys, fields, CIPRuntime.getOperateSubject());
            msg.errorCode = CIPErrorCode.CALL_SUCCESS.code;
            msg.msg = CIPErrorCode.CALL_SUCCESS.name;
        } else {
            msg.errorCode = CIPErrorCode.ERROR_INVALID_AUTHORIZATION.code;
