...
public abstract class BMS_org_relationDaoImpl implements BMS_org_relationDao {

    // dataMapper
    protected BeanPropertyRowMapper<BMS_org_relationData> dataRM;
    // jdbcTemplate
    @Autowired
    protected JdbcTemplate jdbcTemplate;
    // poMapper
    protected BeanPropertyRowMapper<BMS_org_relationPO> poRM;
    // tableKeys
    ...
    public BMS_org_relationDaoImpl() {
        this.tableName = "bms_org_relation";
        this.tableKeys = new String[]{
                "sup_org_id",
                "org_id"
        };
        this.dataRM = BeanPropertyRowMapper.newInstance(BMS_org_relationData.class);
        this.poRM = BeanPropertyRowMapper.newInstance(BMS_org_relationPO.class);
    ...
    public BMS_org_relationPO getSingle(Object... keys) {
        ...
        try {
            ...
            sql.delete(sql.length() - 5, sql.length());
            List<BMS_org_relationPO> pos = jdbcTemplate.query(sql.toString(), keys, poRM);
            if (pos != null && pos.size() > 0)
                return pos.get(0);
