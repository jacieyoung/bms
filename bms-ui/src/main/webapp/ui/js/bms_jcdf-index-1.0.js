...
$(function () {
    ...
    $('#appHeader').html(appTitle);
    $('#softCompany').html(softCompany);
...
function openNewTab(id) {
    ...
    if (href == undefined) {
        //autoDisappear('提示消息','该用户没有该权限,请找管理员',2000);
        return false;
    }
    if ($('#centerTab').tabs('exists', title)) {
        ...
        if (href.indexOf(host) > -1) {
            href = href + "&time=" + (new Date().getTime());
        }
        $('#centerTab').tabs('add', {
            id: id,
            title: title,
    ...
    window.top.$(document.body).layout("collapse", "west");
}
...
function openNewTab2(title, href) {
    if ($('#centerTab').tabs('exists', title)) {
        ...
        if (href.indexOf(host)) {
            href = href + "&time=" + (new Date().getMilliseconds());
        }
        $('#centerTab').tabs('add', {
//			id:id,
            title: title,
...
$(function () {
    ...
    // 	jcdfDialog("", 'admin/tms_admin_net_change.html?actionId=tms_admin_net_change', '切换机构', 350, 550, 0.98);
    // });
    /**
...
function generateMenu(menus) {
    ...
    for (var i = 0; i < menus.length; i++) {
        ...
        if (menus[i].type == "F" && menus[i].level == 0) {
            ...
            if (menus[i].url.indexOf("@#host#@") > -1) {
                var menus_url = menus[i].url.replace(/@#host#@/g, window.location.host);

                lis += "<li iconCls='" + iconCls + "'>" +
                    "<a id=" + menus[i].id + " link='" + menus_url + "' onclick=openNewTab('" + menus[i].id + "') ><div style='width:160px'>" + menus[i].name + "</div></a>" +
                    "</li>";
            } else if (menus[i].url.indexOf("http:") > -1) {
                lis += "<li iconCls='" + iconCls + "'>" +
                    "<a id=" + menus[i].id + " link='" + menus[i].url + "' onclick=openNewTab('" + menus[i].id + "') ><div style='width:160px'>" + menus[i].name + "</div></a>" +
                    "</li>";
            } else {
                lis += "<li iconCls='" + iconCls + "'>" +
                    "<a id=" + menus[i].id + " link='../../" + menus[i].url + "' onclick=openNewTab('" + menus[i].id + "') ><div style='width:160px'>" + menus[i].name + "</div></a>" +
...
//以tab的形式打开一个模块
function openNewTabAdapter(id, title, href, sysId) {
    id = "frame_" + id;
    var centerHeight = window.top.$("#welcomeTab").parent().height() - 4;
    if (window.top.$('#centerTab') != null && window.top.$('#centerTab').length > 0) {
        if (window.top.$('#centerTab').tabs('exists', title)) {
            window.top.$('#centerTab').tabs('select', title);
        } else {
            window.top.$('#centerTab').tabs('add', {
                id: id,
                title: title,
                content: '<iframe id="' + id + '" name="' + id + '" width="100%" height="' + centerHeight + '" src="' + href + '" frameborder="0" scrolling="auto" marginheight="0" marginwidth="0"></iframe>',
                closable: true
            });
        }
    } else {
...
function checkNavigationPage() {
    var url = appId + "/actions/bms_navigation_page/getRouteTypeData.do?actionId=bms_navigation_page_getRouteTypeData";
    $.ajax({
        type: "POST",
        url: url,
        dataType: "json",
        ...
        success: function (oData) {
            if (oData && oData.errorCode == 0) {
                $("#routeName").html(oData.data['org_id_name']);
            } else {
                $$.showJcdfMessager('提示消息', oData.msg, 'warning');
            }
        }
