...
@RestController(value = "BMS_org_personController")
@RequestMapping(value = "/actions/bms_org_person")
public class BMS_org_personControllerImpl implements BMS_org_personController {
    protected Logger log = LoggerFactory.getLogger(getClass());


    // 本地控制实现
    @Autowired
    private BMS_org_personController actionComponent;
     ...
     *     获取下拉框数据
     ******************************************************************/
    @RequestMapping(value = "/loaderData")
    public CIPResponseMsg loaderData(HttpServletRequest request) {

        CIPResponseMsg msg = null;
        try {
            msg = actionComponent.loaderData(request);
        } catch (CIPServiceException e) {
            CIPErrorCode error = e.getErrorCode();
            msg = new CIPResponseMsg();
            msg.errorCode = error.code;
